...
namespace PgTools {
    ...
    template<typename uint_read_len, typename uint_reads_cnt, typename uint_pg_len, class PseudoGenomeClass>
    void DefaultPgMatcher<uint_read_len, uint_reads_cnt, uint_pg_len, PseudoGenomeClass>::exactMatchPg(const string& text,
            ofstream &offsetsDest, uint32_t minMatchLength, bool textFromSamePg) {
        clock_checkpoint();
        const uint_read_len_max readLength = readsList->getMaxReadLength();
