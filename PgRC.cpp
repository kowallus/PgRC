...
void divideGenerateAndMatch(uint16_t error_limit_in_promils, string gen_quality_str, bool filterNReads2Bad, string srcFastqFile, string pairFastqFile,
    ...
    bool qualityDivision = error_limit_in_promils < 1000;
    ...
    if (skipStages < ++stageCount && endAtStage >= stageCount) {
        ...
        else goodReadsIterator = ReadsSetPersistence::createManagedReadsIterator(
                    srcFastqFile, pairFastqFile);
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "m:M:p:S:E:q:rsnNitaA?")) != -1) {
        char* valPtr;
        switch (opt) {
            case 'r':
            ...
            case 'M':
                ...
                break;
            case 'q':
                error_limit_in_promils = atoi(optarg);
                break;
            case 'p':
    ...
    if (revComplPairFile && optind != argc - 3) {
    ...
    divideGenerateAndMatch(error_limit_in_promils, gen_quality, filterNReads2Bad, srcFastqFile, pairFastqFile, ignoreNReads,
